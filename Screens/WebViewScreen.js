...
export default function WebViewScreen({ route, navigation}){
    ...
    const sendEmail = () => {
        ...
        .then(function(response) {
          console.log(`SUCCESS! Email enviado a ${currentUser.email}`, response.status, response.text);
        }, function(error) {
        ...
        setTimeout(() => {
            ...
            alert('Te enviamos un mail con la confirmacion y datos de tu Reserva, muchas gracias !')
        }, 5100);
    }
    const reservaToDB = async () => {
        ...
        try {
            ...
            console.log(`Reserva en Resto: ${empresaDetail.title}, con id: ${reservation.idReserva} para el usuario ${currentUser.name}`)
        } catch (err) {
            console.log(err);
        }
        try {
            ...
            console.log(`Reserva actualizada en User: ${currentUser.name}, con id: ${reservation.idReserva}`)
        } catch (err) {
            console.log(err);
        }
